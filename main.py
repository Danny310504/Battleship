...
def endScreen(window):
    ...
    # Check who won
    player_won = checkForWinners(cGameLogic)
    winner_text = displayWinner("player" if player_won else "computer")
    window.blit(winner_text, (SCREENWIDTH // 2 - winner_text.get_width() // 2,
                              SCREENHEIGHT // 2 - winner_text.get_height() // 2))
...
def displayWinner(winner):
    """Create text surface for winner message"""
...
while RUNGAME:
    ...
    if GAMESTATE == 'Deployment' and DEPLOYMENT != True:
        ...
        if player1Wins == True or computerWins == True:
            GAMESTATE = STAGE[2]
            # Use existing sounds instead of missing victory/defeat sounds
            if player1Wins:
                HITSOUND.play()
            else:
                MISSSOUND.play()
